...
int remover_elemento_com_prioridade(tipo_lista *lst){
    no *c = lst->primeiro, *ant = NULL; //c percorre, ant fica antes

    int prioridade;

    while(c != NULL){
        if (c->idade > prioridade){ //registra a idade maior
            prioridade =  c->idade;
        }
    ...
    while(c != NULL && c->idade != prioridade){ //encontra o elemento com maior idade
        ant = c; //Atualiza anterior
        c = c->prox; //Avança
    }
...
int main(){
    ...
    do{
        ...
        printf("4 - Remover Elemento (Prioridade)\n");
        printf("0 - Sair\n");
        ...
        if(opcao == 1){
            ...
            int cod;
            if(remover_elemento_com_prioridade(&lista)){
