/* Faça uma função que retorno 1 caso a lista seja circular ou 0 caso não seja */
...
int verifica_palindromo(tipo_lista *lst){

    if(lst -> primeiro == NULL || lst -> primeiro == lst -> ultimo){
        return 1;
    }

    celula *esq = lst -> primeiro;
    celula *dir = lst -> ultimo;

    while(esq != NULL && dir != NULL && esq != dir && dir -> prox != esq){
        if(esq -> codigo != dir -> codigo){
            return 0;
        }
        esq = esq -> prox;
        dir = dir -> ant;
...
int lista_circular(tipo_lista *lst){

    if(lista_vazia(lst)) return 0;

    if(lst -> ultimo -> prox == lst -> primeiro && lst -> primeiro -> ant == lst -> ultimo){
        return 1;
    }
    return 0;
}

void tornar_circular(tipo_lista *lst){

    if(lista_vazia(lst)) return;

    lst -> ultimo -> prox = lst -> primeiro;
    lst -> primeiro -> ant = lst -> ultimo;
}
...
int main(){
    ...
    do{
        printf("\n\t0 - Sair e Liberar Memoria\n\t1 - Inserir Fim\n\t2 - Imprimir Lista\n\t3 - Buscar Elemento\n\t4 - Remover Elemento\n\t5 - Contar Elementos\n\t6 - Criar Lista com Codigo\n\t7 - Verificar se eh palindromo\n\t8 - Se a lista eh circular ou nao\n\t9 - Tornar lista circular\n");
        scanf("%d", &opcao);
        ...
        if (opcao == 1) {
            ...
            if(lista_circular(&lista)){
                printf("A lista e circular\n");
            }else{
                printf("A lista NAO e circular.\n");
            }
        }
        else if(opcao == 9){
            tornar_circular(&lista);
            printf("A lista foi transformada em circular com sucesso!\n");
        }
        else if (opcao == 0) {
            liberar_lista(&lista);
