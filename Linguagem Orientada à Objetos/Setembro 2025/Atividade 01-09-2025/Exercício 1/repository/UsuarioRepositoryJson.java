...
public class UsuarioRepositoryJson implements UsuarioRepository {
    ...
    public boolean removerPorCodigo(int codigo) {
        ...
        MensagemRepositoryJson mensagemRepo = MensagemRepositoryJson.getInstancia();
        boolean temMensagens = mensagemRepo.listar().stream()
                .anyMatch(m -> m.getRemetente().getCodigo() == codigo
                || m.getDestinatario().getCodigo() == codigo);
    ...
    public boolean removerPorCodigoEmCascata(int codigo) {
        ...
        MensagemRepositoryJson mensagemRepo = MensagemRepositoryJson.getInstancia();
        List<Mensagem> mensagens = mensagemRepo.listar();

        mensagens.stream()
                .filter(m -> m.getRemetente().getCodigo() == codigo || m.getDestinatario().getCodigo() == codigo)
                .map(Mensagem::getCodigo)
                .forEach(mensagemRepo::removerPorCodigo);
